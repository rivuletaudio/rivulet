...
class AudioTranscode:
    ...
    def _decode_stdin(self, filetype, decoder=None):
    ...
    def transcode_stream_from_generator(self, gen, oldformat, newformat, bitrate=None,
                         encoder=None, decoder=None):
        """returns a generator wih the bytestream of the encoded audio
        stream"""
        self.check_encoder_available(newformat)
        ...
        encoder_process = None
        try:
            decoder_process = self._decode_stdin(oldformat, decoder)
            encoder_process = self._encode(newformat, decoder_process,
                                           bitrate=bitrate, encoder=encoder)
            def writer(gen, decoder_process):
                for kind, gendata in gen:
                    if kind == 'metadata':
                        print "was metadata in the writer thread! -> BAD"
                    else:
                        decoder_process.stdin.write(gendata)
                decoder_process.stdin.close()

            thread.start_new_thread(writer, (gen, decoder_process))
...
#    at = AudioTranscode()
#    for data in at.transcode_stream_stdin('flac', 'mp3'):
